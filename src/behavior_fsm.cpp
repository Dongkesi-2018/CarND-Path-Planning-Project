...
vector<Vehicle> BehaviorFSM::generate_trajectory(
  ...
  if (state.compare("CS") == 0) {
    ...
    cout << "lane_change_trajectory" << endl;
...
bool BehaviorFSM::get_vehicle_behind(map<int, vector<Vehicle>> predictions,
  ...
  if (found_vehicle && ((ego_.s - max_s) > cal_safe_distance(ego_.v))) {
...
bool BehaviorFSM::get_vehicle_ahead(map<int, vector<Vehicle>> predictions,
  ...
  if (found_vehicle && ((min_s - ego_.s) > cal_safe_distance(ego_.v))) {
    cout << "A: found_vehicle = false" << endl;
    found_vehicle = false;
  }
  trace_exit();
  return found_vehicle;
...
void BehaviorFSM::configure() {
  ...
  dt = ParameterConfig::prediction_interval;
  trace_exit();
}
